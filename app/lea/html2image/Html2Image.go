...
func (this *Html2Image) SetBottom(username, url string) {
    ...
    // 左侧写字
    this.NewP()
    this.InsertText("本文来自 " + username + " 的gemsnote笔记", true, "  ")
    this.NewBr()
    this.InsertText("个人博客: ", false, "  ")
    siteUrl, _ := revel.Config.String("site.url")
    if siteUrl == "" {
        siteUrl = "http://gemsnote.com"
    }
...
//    this.painWidth = this.width - 100
//    this.NewP()
//    this.InsertText("gemsnote, 不一样的笔记.", false, "  ")
//    this.NewBr()
...
//    this.InsertText(". 赶紧加入吧! gemsnote.com", false, "")
//
    // Logo
}
...
func (this *Html2Image) setLogo() {
	// 右上角的logo
	path := revel.BasePath + "/public/images/gemsnote/logo-20-a-6.png"
	println(path)
	this.setImage(path, 320, 10)

	// 右下角设置Logo
//	path = revel.BasePath + "/public/images/gemsnote/logo-60-a-6.png"
