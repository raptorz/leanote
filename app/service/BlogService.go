...
func (this *BlogService) GetBlogTags(userId string) []info.TagCount {
	...
	db.ListByQOptions(db.TagCounts, query, &tagCounts, db.QueryOptions{Sort: []string{"-Count"}})
	return tagCounts
}
...
func (this *BlogService) ListBlogsArchive(userId, notebookId string, year, month int, sortField string, isAsc bool) []info.Archive {
	...
	notes := []info.Note{}
	db.ListByQOptions(db.Notes, q, &notes, db.QueryOptions{Sort: []string{sorter}})
...
func (this *BlogService) PreNextBlog(userId string, sorterField string, isAsc bool, noteId string, baseTime interface{}) (info.Post, info.Post) {
	...
	db.GetByQOptions(db.Notes, query, &note, db.QueryOptions{Sort: []string{sortFieldR1}, Limit: 1})
	...
	//	LogJ(query)
	//	Log(sortFieldR2)
	db.GetByQOptions(db.Notes, query, &note2, db.QueryOptions{Sort: []string{sortFieldR2}, Limit: 1})
...
func (this *BlogService) ListLikedUsers(noteId string, isAll bool) ([]info.UserAndBlog, bool) {
	...
	if isAll {
		db.ListByQOptions(db.BlogLikes, query, &likes, db.QueryOptions{Sort: []string{sortFieldR}, Skip: skipNum, Limit: pageSize})
	} else {
		db.ListByQOptions(db.BlogLikes, query, &likes, db.QueryOptions{Sort: []string{sortFieldR}})
	}
...
func (this *BlogService) ListComments(userId, noteId string, page, pageSize int) (info.Page, []info.BlogCommentPublic, map[string]info.UserAndBlog) {
	...
	db.ListByQOptions(db.BlogComments, query, &comments2, db.QueryOptions{
		Sort: []string{sortFieldR}, Skip: skipNum, Limit: pageSize,
	})
