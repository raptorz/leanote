...
func (this *ShareService) GetShareNotebooks(userId string) (info.ShareNotebooksByUser, []info.User) {
	...
	db.ListByQOptions(db.ShareNotebooks, q, &shareNotebooks, db.QueryOptions{Sort: []string{"-ToUserId"}}) // 按ToUserId降序排序, 那么有ToUserId的在前面
...
func (this *ShareService) ListShareNotesByNotebookId(notebookId, myUserId, sharedUserId string,
	...
	db.ListByQOptions(db.ShareNotes, q, &shareNotes, db.QueryOptions{Sort: []string{"-ToUserId"}}) // 给个的权限>给组织的权限
...
func (this *ShareService) ListShareNotes(myUserId, sharedUserId string,
	...
	db.ListByQOptions(db.ShareNotes, q, &shareNotes, db.QueryOptions{
		Sort: []string{"-ToUserId"}, Skip: skipNum, Limit: pageSize,
	})
...
func (this *ShareService) AddShareNotebook1(shareNotebook info.ShareNotebook) bool {
	...
	if shareNotebook.ShareNotebookId == "" {
		shareNotebook.ShareNotebookId = bson.NewObjectId()
	}
...
func (this *ShareService) AddShareNotebookToUserId(notebookId string, perm int, userId, toUserId string) (bool, string, string) {
	...
	shareNotebook := info.ShareNotebook{ShareNotebookId: bson.NewObjectId(), NotebookId: bson.ObjectIdHex(notebookId),
		UserId:      bson.ObjectIdHex(userId),
		ToUserId:    bson.ObjectIdHex(toUserId),
		Perm:        perm,
...
func (this *ShareService) AddShareNoteToUserId(noteId string, perm int, userId, toUserId string) (bool, string, string) {
	...
	shareNote := info.ShareNote{ShareNoteId: bson.NewObjectId(), NoteId: bson.ObjectIdHex(noteId),
		UserId:      bson.ObjectIdHex(userId),
		ToUserId:    bson.ObjectIdHex(toUserId),
		Perm:        perm,
		CreatedTime: time.Now(),
...
func (this *ShareService) HasUpdatePerm(userId, updatedUserId, noteId string) bool {
	...
	db.ListByQOptions(db.ShareNotes, q, &shares, db.QueryOptions{Sort: []string{"-ToUserId"}}) // 个人 > 组织
	...
	db.ListByQOptions(db.ShareNotebooks, q, &shares2, db.QueryOptions{Sort: []string{"-ToUserId"}}) // 个人 > 组织
...
func (this *ShareService) AddShareNoteGroup(userId, noteId, groupId string, perm int) bool {
	...
	// MongoDB generates _id when it is omitted, but PostgreSQL requires the
	// application to provide the CHAR(24) primary key used by the shared model.
	shareNote := info.ShareNote{ShareNoteId: bson.NewObjectId(), NoteId: bson.ObjectIdHex(noteId),
...
func (this *ShareService) AddShareNotebookGroup(userId, notebookId, groupId string, perm int) bool {
	...
	// Keep group shares consistent with personal shares on PostgreSQL: _id is
	// generated by MongoDB but must be present for the PostgreSQL primary key.
	shareNotebook := info.ShareNotebook{ShareNotebookId: bson.NewObjectId(), NotebookId: bson.ObjectIdHex(notebookId),
		UserId:      bson.ObjectIdHex(userId), // 冗余字段
		ToGroupId:   bson.ObjectIdHex(groupId),
		Perm:        perm,
