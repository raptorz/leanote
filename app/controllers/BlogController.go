...
func (c Blog) setPreviewUrl() {
	...
	blogUrl := "/preview" // blog.gemsnote.com

	indexUrl = blogUrl + "/" + userIdOrEmail
	cateUrl = blogUrl + "/cate/" + userIdOrEmail // /notebookId

	postUrl = blogUrl + "/post/" + userIdOrEmail        // /xxxxx
	searchUrl = blogUrl + "/search/" + userIdOrEmail    // blog.gemsnote.com/search/userId
	singleUrl = blogUrl + "/single/" + userIdOrEmail    // blog.gemsnote.com/single/singleId
	archiveUrl = blogUrl + "/archives/" + userIdOrEmail // blog.gemsnote.com/archive/userId
	tagsUrl = blogUrl + "/tags/" + userIdOrEmail        // blog.gemsnote.com/archive/userId
...
func (c Blog) setUrl(userBlog info.UserBlog, userInfo info.User) {
	// 主页 http://gemsnote.com/blog/life or http://blog.gemsnote.com/life or http:// xxxx.gemsnote.com or aa.com
	// host := c.Request.Request.Host
	// var staticUrl = configService.GetUserUrl(strings.Split(host, ":")[0])
	// staticUrl == host, 为保证同源!!! 只有host, http://gemsnote.com, http://blog/gemsnote.com
	// life.gemsnote.com, lealife.com
	siteUrl := configService.GetSiteUrl()
...
func (c Blog) setBlog(userBlog info.UserBlog, userInfo info.User) {
	blogInfo := map[string]interface{}{
		...
		"CommentType": userBlog.CommentType, // gemsnote, or disqus
