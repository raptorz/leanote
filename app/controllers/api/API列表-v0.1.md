...
Content中的数据, 图片,附件在Gemsnote的链接是, 所以, 不管你在本地的笔记中是以什么形式来保存图片,附件的链接的,请addNote和updateNote时务必将链接修改成gemsnote服务器上的链接.
http://gemsnote.com/api/file/getImage?fileId=xx
单个附件: 
http://gemsnote.com/api/file/getAttach?fileId=xx
所有附件:
http://gemsnote.com/api/file/getAllAttachs?noteId=xxx
...
图片在笔记内的链接必须为: http://gemsnote.com/api/file/getImage?fileId=LocalFileId或FileId
附件如果插入到了笔记内容内, 其链接必须为: http://gemsnote.com/api/file/getAttach?fileId=LocalFileId或FileId
...
http://gemsnote.com 不绝对, 因为用户可以自建服务, 所以在开发时需要可配置
