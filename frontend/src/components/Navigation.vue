...
const emit=defineEmits<{showSpaces:[],showNotes:[]}>()
...
<template><nav class="rail" aria-label="全局导航"><RouterLink to="/note" class="mark" title="珠玑笔记">◈</RouterLink><button v-if="workspace" class="rail-action" title="文章列表" aria-label="文章列表" @click="emit('showNotes')">▤</button><RouterLink v-else to="/note" title="笔记">笔记</RouterLink><button v-if="showSpacesButton" class="rail-action" title="展开我的空间" aria-label="展开我的空间" @click="emit('showSpaces')">☷</button><RouterLink v-if="admin" to="/admin" title="系统管理">管理</RouterLink><div class="rail-bottom"><div class="user-menu" @focusout="focusOut" @keydown.escape="close"><button class="avatar-button" :aria-expanded="menuOpen" aria-haspopup="menu" aria-label="用户菜单" @click="menuOpen=!menuOpen"><img v-if="avatarSrc" :src="avatarSrc" alt=""><span v-else>{{initials}}</span></button><div v-if="menuOpen" class="user-popover" role="menu"><RouterLink to="/member" role="menuitem" @click="close">账号</RouterLink><a href="/logout" role="menuitem">退出</a></div></div></div></nav></template>
